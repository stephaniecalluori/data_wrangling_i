...
Let’s import the ‘FAS_litters.csv’ csv using a relative path. Tidyverse
using readr package (use for csv files). Readxl use for excel files.
...
Let’s import the ‘FAS_pups.csv’ csv using a relative path. Janitor clean
names makes all variable names snakecase.
...
Look at a data summary. Structure tells you column, size, col types.
Skim has neat summary of data; helpful to skim looking at dataset for
first time.
...
\##Import other types of files. Import a xlsx file first.
...
\##Exporting data We have code that cleans data and need to export it.
